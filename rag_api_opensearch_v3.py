...
def router_node(state: GraphState) -> Dict[str, Any]:
    ...
    # 간단한 프롬프트로 분류 (JSON 파싱 대신 텍스트 매칭)
    simple_prompt = f"""사용자 질문을 분류하세요. 아래 세 가지 중 하나만 답하세요:

- search: 메일 내용 검색이 필요 (수율, 이슈, 문제, 분석, 개선, 진행상황 등)
- statistics: 통계/수치 조회 필요 (몇 개, 제출 수, 미제출 팀, 현황 등)
- general: 일반 대화 (인사, 감사, 도움말 등)
...
분류 결과 (search, statistics, general 중 하나만):"""
    ...
    try:
        ...
        print(f"🔀 [Router] 분류 결과: {route} (LLM 응답: {answer[:50]})")
    except Exception as e:
...
- get_missing_teams: 주간보고 미제출 팀 조회 (week 파라미터 필수, 예: 2025-48)
- get_mail_type_summary: 전체 메일 현황 요약
- get_available_weeks: 데이터가 있는 주차 목록

## 규칙
1. 질문에 맞는 함수를 반드시 1개 이상 호출하세요.
2. "미제출 팀"은 get_missing_teams를 사용하고, week 파라미터를 지정하세요.
3. "주간보고 몇 개", "제출 수"는 count_weekly_reports_by_team을 사용하세요.
4. 모호한 질문은 get_mail_type_summary를 사용하세요.
"""
...
def statistics_node(state: GraphState) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.tool_calls:
            ...
            # Tool 호출이 없으면 기본으로 전체 요약 조회
            print("⚠️ [Statistics] LLM이 Tool을 호출하지 않음, 기본값 사용")
