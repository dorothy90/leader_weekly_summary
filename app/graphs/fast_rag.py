...
class FastRAGWorkflow:
    def __init__(
        ...
        general_timeout_seconds: float | None = None,
    ):
        ...
        self.model_step_timeout_seconds = max(
            ...
            float(
                ...
                if model_step_timeout_seconds is not None
                else min(5.0, self.deadline_seconds)
            ),
        )
        self.general_timeout_seconds = max(
            0.001,
            float(
                general_timeout_seconds
                if general_timeout_seconds is not None
                else min(15.0, self.deadline_seconds)
            ),
        )
    ...
    async def respond_general(
        ...
        try:
            if is_identity_question(request.message):
            ...
            elif name := declared_name(request.message):
                answer = f"반갑습니다, {name}님. 이름을 기억하겠습니다."
            elif is_name_recall_question(request.message) and (
                ...
                if conversation is not None and hasattr(
                    ...
                    async with asyncio.timeout(self.general_timeout_seconds):
                        answer = await self.llm.complete_text(GENERAL_SYSTEM, user)
            result = FastRAGResult(
