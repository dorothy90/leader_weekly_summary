...
class ChatModel:
    ...
    def complete(self, stage, system, payload, schema):
        ...
        if len((system+user).encode()) > self.settings.max_input_bytes:
            trace_event('request',dict(model=self.settings.llm_model,max_tokens=self.settings.max_output_tokens,
                        messages=[dict(role='system',content=system),dict(role='user',content=user)],not_sent=True))
        ...
        for attempt in range(3):
            try:
                ...
                trace_event('request',request_body)
                started = time.monotonic()
                response = self.client.post(self.settings.llm_url.rstrip('/')+'/chat/completions', json=request_body)
                trace_event('response',dict(status_code=response.status_code,body=response.text,
                ...
                parsed = schema.model_validate(json.loads(content)).model_dump()
                trace_event('parsed_result',parsed)
                return parsed
            except httpx.HTTPStatusError as exc:
                raise LLMError(f'모델 호출 실패 (HTTP {exc.response.status_code}). 모델 주소·이름·인증 설정을 확인하세요.') from None
            except httpx.TransportError as exc:
                trace_event('error',dict(type=type(exc).__name__,message='모델 서버 연결 오류'))
                if attempt == 2:
                    raise LLMError('모델 서버에 연결할 수 없습니다. 연결 설정을 확인한 뒤 다시 시도하세요.') from None
                time.sleep(attempt+1)
            except (IndexError, KeyError, TypeError, ValueError) as exc:
                trace_event('error',dict(type=type(exc).__name__,message=str(exc)))
