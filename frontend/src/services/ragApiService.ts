...
const isExecution = (value: unknown) =>
  isRecord(value) &&
  ['succeeded', 'limited', 'failed'].includes(String(value.status)) &&
  isNullableString(value.failure_stage) &&
  isNullableString(value.error_code) &&
  typeof value.retryable === 'boolean' &&
  Number.isInteger(value.search_count) && Number(value.search_count) >= 0 &&
  ...
  typeof value.include_in_llm_history === 'boolean'
