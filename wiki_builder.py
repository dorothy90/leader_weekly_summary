...
**2. 수율 주요내용 (6)**  ← 각 팀 한 줄: 정확히 1~2 핵심, 60자 이하, 구분자 1번
- **Spica수율**: 한달 핵심 수치 1개 + 가장 큰 이슈/리스크 1개 (분석 기법/액션 서술 금지).
- **HBM수율**: 한달 핵심 수치 1개 + 가장 큰 이슈/리스크 1개.
- **LC_CP수율**: 한달 핵심 수치 1개 + 가장 큰 이슈/리스크 1개.
- **Olympus수율**: 한달 핵심 수치 1개 + 가장 큰 이슈/리스크 1개.
- **CL_PE수율**: 한달 핵심 수치 1개 + 가장 큰 이슈/리스크 1개.
- **우시수율PTE**: 한달 핵심 수치 1개 + 가장 큰 이슈/리스크 1개.

**3. 품질 주요내용 (2)**  ← 각 팀 한 줄: 정확히 1~2 핵심, 60자 이하, 구분자 1번
- **DRAM품질PTE**: 한달 핵심 수치 1개 + 가장 큰 이슈/리스크 1개.
- **NAND품질PTE**: 한달 핵심 수치 1개 + 가장 큰 이슈/리스크 1개.

**4. 증산TF수율분과 (4)**  ← 각 팀 한 줄: 증산TF 한정, 정확히 1~2 핵심, 60자 이하, 구분자 1번
- **DRAM수율전략**: 증산TF(증산 목표·라인 확장·신규 라인 셋업·증산 ramp) 한정 — 수치 1 + 이슈/리스크 1.
- **DRAM FA PTE**: 증산TF 한정 (증산 라인 FA 지원·증산 ramp 분석 등) — 수치 1 + 이슈/리스크 1.
- **NAND수율전략**: 증산TF 한정 — 수치 1 + 이슈/리스크 1.
- **NAND FA PTE**: 증산TF 한정 — 수치 1 + 이슈/리스크 1.

**5. 개발제품수율 및 양산성 (4)**  ← 각 팀 한 줄: 정확히 1~2 핵심, 60자 이하, 구분자 1번
- **DRAM SRT 개발공정**: HBM4E 한정 — 수치 1 + 이슈/리스크 1.
- **Heraion양산수율**: 수치 1 + 이슈/리스크 1.
- **Procyon양산수율**: 수치 1 + 이슈/리스크 1.
- **Robson양산수율**: 수치 1 + 이슈/리스크 1.
...
8. 섹션 2~5 의 각 팀 한 줄에는 **정확히 1~2 핵심만** 담는다. 3 핵심 이상은 절대 금지(작성 후 3 핵심이 보이면 즉시 가장 덜 중요한 1개를 삭제).
   - 우선순위: (i) 한달 핵심 수치/지표 1개, (ii) 가장 큰 이슈/리스크 1개. 이 둘 외에는 무엇이 있어도 추가하지 말 것.
   - 부수 활동·분석 기법·후속 액션 나열 절대 금지(예: `DOE 진행`, `Pareto 분석`, `학습곡선 가속`, `EPM PCSA 안정화` 같은 액션/방법론 서술 금지 — 수치 또는 이슈/리스크 한 마디로 끝).
   - 두 핵심을 잇는 구분자는 한 줄에 **`;` 또는 `,` 단 1번**만 허용. 두 번 이상 사용 시 위반.
   - 한 줄 길이는 **한국어 60자 이하**(공백 포함). 60자 초과 시 가장 덜 중요한 부분을 삭제해 줄임.
   - 예시: ❌ `수율 91.0~92.4% 등락, Etch 잔류물 및 ECC fail 이슈 반복, D0 추세 및 Bridge defect Pareto 분석 진행` (3 핵심, 60자 초과)
   - 예시: ✅ `수율 91.0~92.4%; Etch 잔류물·ECC fail 이슈 반복` (수치 1 + 이슈 1, 60자 이하)
